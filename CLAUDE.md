...
| Route                  | Purpose                                                    |
| ---------------------- | ---------------------------------------------------------- |
| `/`                    | Hero, intro, coverage grid, how-it-works, community, CTA.   |
| `/about`               | Owner intro, why Farm Bureau, how I work, service area.     |
| `/coverage`            | Hub linking the four lines of business.                     |
| `/coverage/[slug]`     | auto, home, life, business — generated from data.           |
| `/quote`               | Lead-capture form + what-happens-next. No rate estimator.   |
| `/resources`           | Index of educational posts.                                 |
| `/resources/[slug]`    | Individual post, from Markdown.                             |
| `/contact`             | Form, contact info, hours, location block.                  |
| `/thank-you`           | No-JS form fallback target. `noindex`.                      |
| `/404`                 | Not found.                                                  |
...
`Button`, `Card`, `Section`, `SectionHeading`, `CTASection`, `LeadForm`,
`CoverageIcon`, `Header`, `Footer`.

Gotchas learned the hard way:

- **Never pass Tailwind color classes to `<Button class="…">` to recolor it** —
  they collide with the variant's own classes and you get white-on-white. Add a
  variant instead. `inverse` is the one for dark/brand backgrounds.
- **Don't put HTML entities inside prop strings** (`title="…if&hellip;"`) — props
  are text, so it renders literally. Use the real character (`…`, `—`).
- Astro can drop the space between a line-ending word and a following inline
  `<a>`. Use `&#32;` when a link starts a new source line mid-sentence.
...
- Mobile nav is a `<details>` element rather than a JS drawer, so it works
  before any script runs.
- The contact page location block links out to Google Maps instead of embedding
  an iframe (~1MB of third-party JS and cookies for a picture of a street).
- Placeholder photos in `src/assets/` are generated by
  `scripts/make-placeholder-images.mjs`. Replace with real photography and
  delete the script.
- `zod` is a direct dependency; import `z` from `'zod'`, not from
  `astro:content` (deprecated in Astro 7).

## Verify before calling it done

```
npm run build && npx astro check   # expect 0 errors
```

Then check a page at 390px wide as well as desktop. Note that full-page
screenshots don't trigger lazy images — scroll first, or you'll "find" a bug
that isn't there.
